...
class Api_Tz extends PhalApi_Api
{
    ...
    public function phpInfoContent()
    {
        ob_start();
        phpinfo();
        $php_info_content = ob_get_clean();
        die($php_info_content);
    }
    ...
    public function info()
    {
        ...
        DI()->view->assign(['sysInfo' => $sys_info['sysInfo'], 'sysReShow' => $sys_info['sysReShow'], 'phpSelf' => $phpSelf, 'cpu_show' => $cpu_show]);
        DI()->view->show('tz');
    }
    ...
    public function doTest()
    {
        ...
        switch ($post['type']) {
            ...
            case 'pSpeed':
                ...
                break;
            case 'pMySQL':
                return ['type' => $post['type'], 'result' => Domain_Tz::testMySQLi($post)];
                break;
            case 'pFun':
                return ['type' => $post['type'], 'result' => Domain_Tz::testFun($post)];
                break;
            case 'pMail':
                return ['type' => $post['type'], 'result' => Domain_Tz::testMail($post)];
                break;
            default:
