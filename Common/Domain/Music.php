...
class Domain_Music
{
	...
	function maicong_curl($args = array())
	{
		...
		$args = $this->maicong_parse_args($args, $default);
	...
	function maicong_get_song_by_name($query, $site = 'netease')
	{
		...
		$radio_search_url = $this->maicong_song_urls($query, 'query', $site);
		if (empty($query) || empty($radio_search_url)) {
			return;
		}
		$radio_result = $this->maicong_curl($radio_search_url);
		...
		return $this->maicong_get_song_by_id($radio_songid, $site, true);
	}
	...
	function maicong_get_song_by_id($songid, $site = 'netease', $multi = false)
	{
		...
		if ($multi) {
			...
			foreach ($songid as $key => $val) {
				$radio_song_urls[] = $this->maicong_song_urls($val, 'songid', $site);
			}
		} else {
			$radio_song_urls[] = $this->maicong_song_urls($songid, 'songid', $site);
		}
		...
		foreach ($radio_song_urls as $key => $val) {
			$radio_result[] = $this->maicong_curl($val);
		}
			...
			case 'kugou':
				foreach ($radio_result as $key => $val) {
					...
					if (!empty($radio_detail) && $radio_data['status']) {
						...
						$radio_imginfo = json_decode($this->maicong_curl($radio_img), true);
				...
				foreach ($radio_result as $key => $val) {
					...
					if (!empty($radio_detail)) {
						...
						$radio_songs[] = array(
							...
							'music' => $this->maicong_decode_xiami_location($radio_detail[0]['location']),
			...
			case 'qingting':
				foreach ($radio_result as $key => $val) {
					...
					if (!empty($radio_detail)) {
						...
						$radio_channels_info = json_decode($this->maicong_curl($radio_channels), true);
			...
			case 'soundcloud':
				foreach ($radio_result as $key => $val) {
					...
					if (!empty($radio_detail)) {
						...
						$radio_streams_info = json_decode($this->maicong_curl($radio_streams), true);
	...
	function maicong_get_song_by_url($url)
	{
		...
		if (!empty($match_netease)) {
			...
			$match_request = $this->maicong_curl($match_resolve);
		...
		return $this->maicong_get_song_by_id($songid, $songtype);
	}
