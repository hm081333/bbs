 ...
 * Time: 下午 10:13
 */
class Plugin_Event implements Wechat_Plugin_Event
{
	public function handleEvent($inMessage, &$outMessage)
	{
		...
		switch ($inMessage->getEvent()) {
			...
			case 'LOCATION':
				...
				$rs = Common_Function::baidu_map($Latitude . ',' . $Longitude, 'location');
				$outMessage = new Wechat_OutMessage_Text();
				$outMessage->setContent('您现在位于' . $rs['result']['formatted_address']);
				break;
			case 'click':
