...
class Curl
{
     ...
     * @throws Exception
     */
    public function __construct($file = '', array $head = [])
    {
...
// 参见：http://www.laruence.com/2014/01/21/2939.html
     ...
     * @return $this
     */
    public function init($file = '', array $head = ['User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36'])
    {
        $this->reset();
        if (!empty($file)) {
            $this->setUrl($file);
        }
        $this->setHeader($head)->setAll([//curl默认设定
