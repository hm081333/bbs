...
class QQLogin
{
    ...
    private function get_curl($url, $post = false, $referer = false, $cookie = false, $header = false, $ua = false, $nobaody = false, $split = false)
    {
        $ch = curl_init();
     ...
     */
    public function wxLogin(string $uuid = '', string $last = '')
    {
        if (empty($uuid)) {
        ...
        if (preg_match("/wx_errcode=(\d+);window.wx_code=\'(.*?)\'/", $ret, $match)) {
            ...
            if ($errcode == 408) {
                ...
                throw new Exception('请在微信中点击确认即可登录', 2);
            } else if ($errcode == 402) {
                // return ['code' => '3', 'msg' => '二维码已失效'];
                throw new Exception('二维码已失效', 3);
            } else if ($errcode == 405) {
                ...
                if ($mkey = $mkey[1]) {
                    ...
                    throw new Exception('登录成功，获取mkey失败！', -1);
                }
                if ($bduss[1] && $stoken[1] && $ptoken[1]) {
                    // return ['code' => 0, 'uid' => $this->getUserid($uname[1]), 'user' => $uname[1], 'displayname' => $displayname[1], 'bduss' => $bduss[1], 'ptoken' => $PTOKEN[1], 'stoken' => $STOKEN[1]];
                    return [
                        'name' => $displayname[1],
                        'bduss' => $bduss[1],
                    ];
                    ...
                    throw new Exception('登录成功，回调百度失败！', -1);
                }
            } else {
    ...
    public function getQqQrCode()
    {
        ...
        $arr = $this->get_curl($url, false, false, false, true, false, false, true);
     ...
     */
    public function qrLogin(string $qrsig = '')
    {
        if (empty($qrsig)) {
            throw new BadRequestException('qrsig不能为空');
        }
            ...
            if ($r[0] == 0) {
                ...
                if ($data) {
                    ...
                    if ($mkey = $mkey[1]) {
                        ...
                        $data = $this->get_curl($url, $post, false, $cookie, true);
                        preg_match("/Location: (.*?)\r\n/", $data, $match);
                        if ($match[1]) {
                            ...
                            throw new Exception("登录成功，回调百度失败！", 6);
                        }
                    } else {
                        // exit('ptuiCB("6","","登录成功，获取mkey失败！");');
                        throw new Exception("登录成功，获取mkey失败！", 6);
                    }
                }
                if ($bduss[1] && $stoken[1] && $ptoken[1]) {
                    ...
                    throw new Exception("登录成功，获取相关信息失败！", 6);
                }
            } else if ($r[0] == 65) {
                // exit('ptuiCB("1","","二维码已失效。");');
                throw new Exception("二维码已失效。", 1);
            } else if ($r[0] == 66) {
                // exit('ptuiCB("2","","二维码未失效。");');
                throw new Exception("二维码未失效。", 2);
            } else if ($r[0] == 67) {
                // exit('ptuiCB("3","","正在验证二维码。");');
                throw new Exception("正在验证二维码。", 3);
            } else {
                // exit('ptuiCB("6","","' . str_replace('"', '\'', $r[4]) . '");');
                throw new Exception(str_replace('"', '\'', $r[4]), 6);
            }
        } else {
            // exit('ptuiCB("6","","' . $ret . '");');
            throw new Exception($ret, 6);
        }
    }
    ...
    {
        $data = curl()->post('http://api.cccyun.cc/api/qrcode_noauth.php', ['image' => urlencode($image)]);
        $arr = json_decode($data, true);
        if ($arr['code'] == 1) {
            return [
                "code" => 0,
            ...
            throw new InternalServerErrorException($arr['msg']);
        } else {
