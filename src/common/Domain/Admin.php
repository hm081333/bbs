...
class Admin
{
    ...
    {
        //将用户信息存入SESSION中
        $_SESSION[ADMIN_TOKEN] = \Common\encrypt(serialize($user));// 保存在session
    }
    ...
    {
        return unserialize(\Common\decrypt($_SESSION[ADMIN_TOKEN] ?? ''));// Session中的管理员信息
    }
    ...
    public static function getCurrentAdmin(bool $thr = false)
    {
        if (!isset(self::$admin)) {
            ...
            if (!$admin) {
                // $admin_token = \PhalApi\DI()->request->getHeader(ADMIN_TOKEN, false);// 获取header中携带的Token
        ...
        if ($thr && !self::$admin) {
            throw new \Exception\BadRequestException(\PhalApi\T('请登录'), 2);
        }
        return self::$admin;
    }
